...
@section('content')
    <div class="responseive-width-full">
        <div id="panels-app" data-env={{ env('APP_ENV') }} data-user="{{ $user }}"
            data-rights="{{ $rights }}" data-locale="{{ Config::get('app.locale') }}"
        ...
        {{-- <div id="panels-app" data-env={{ env('APP_ENV') }} data-user="{{ Auth::user() }}"
            ...
            data-features="{{ $features
    ::whereIn('state', ['installed', 'built-in'])->with('tools.requiredRights', 'tools.operations.requiredRights', 'requiredRights')->get() }}">
        </div> --}}
